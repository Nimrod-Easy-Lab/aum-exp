...
fact twoDistinctFields {
	some disj f1,f2:Field | f1.id != f2.id && f1.type + f2.type in Int_
}

fact allMethodsPublics {
	all m:Method | m.acc = public
}

fact ifDifferentVars {
	all if:If | if.var1 != if.var2 && if.var1.id != if.var2.id
}

run show for 1 Package, 1 Class, 1 ClassId, 2 Method, 2 MethodId, 2 Body, 6 Id, 2 Field, 2 FieldId
